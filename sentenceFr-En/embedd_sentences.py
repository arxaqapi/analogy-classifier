...
def avg_sent_to_vec(sentence, embedding_dict, embedding_size, lower):
    ...
    return vector / (n_words + 0.01)  #added 0.01 to avoid zero division
...
def embedd_row(row, word_embedding_used, sentence_embedding_method, embedding_dict, embedding_size, k):
    if sentence_embedding_method == 'AVG' and word_embedding_used == 'glove':
        ...
        a = avg_sent_to_vec(row[0], embedding_dict,
                            embedding_size, lower=False)
        b = avg_sent_to_vec(row[1], embedding_dict,
                            embedding_size, lower=False)
        c = avg_sent_to_vec(row[2], embedding_dict,
                            embedding_size, lower=False)
        d = avg_sent_to_vec(row[3], embedding_dict,
                            embedding_size, lower=False)
